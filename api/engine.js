...
const OR_SITE  = 'https://academyscosao.vercel.app'; /* v59: domínio Vercel real */
...
async function actionGenerateLesson(payload) {
  ...
  const palavrasPorCap = Math.min(Math.max(parseInt(payload.palavrasPorCap, 10) || 400, 150), 2000); /* v59: default 400, max 2000 */
...
- O último capítulo deve ser "Referências Bibliográficas" sem subtópicos`;
  ...
  if (action === 'gerar_capitulo') {
    ...
    /* v2.2: max_tokens proporcional (~1.7 tokens/palavra português + margem) */
    const maxTokens = Math.min(Math.max(Math.round(palavrasPorCap * 1.7), 400), 8192);
...
Escreve o capítulo completo agora. PÁRA aos ${palavrasPorCap} palavras.`;

    const r = await callOpenRouter([{ role: 'user', content: prompt }], { max_tokens: maxTokens, temperature: 0.65 });
    const rLimpo = removerCabecalhoDuplicado(r, capNum, capTitulo);
    /* v2.2: truncagem de segurança — garantir que não ultrapassa 130% do pedido */
    const rFinal = truncarPorPalavras(rLimpo, Math.round(palavrasPorCap * 1.1));
    return envelope(action, { resposta: rFinal });
  }
...
/* ====================================================================
   v2.2 — TRUNCAGEM POR PALAVRAS
   Garante que o texto não ultrapassa o limite pedido.
   Preserva parágrafos completos — não corta a meio de frase.
==================================================================== */
function truncarPorPalavras(texto, maxPalavras) {
  if (!texto || maxPalavras <= 0) return texto;
  const palavras = texto.split(/\s+/);
  if (palavras.length <= maxPalavras) return texto;

  /* Cortar no limite e depois recuar até ao fim do parágrafo mais próximo */
  const cortado = palavras.slice(0, maxPalavras).join(' ');
  /* Encontrar o último ponto final ou parágrafo */
  const ultimoPonto = Math.max(cortado.lastIndexOf('.\n'), cortado.lastIndexOf('. '), cortado.lastIndexOf('.\n\n'));
  if (ultimoPonto > cortado.length * 0.7) {
    return cortado.substring(0, ultimoPonto + 1).trim();
  }
  return cortado.trim();
}
